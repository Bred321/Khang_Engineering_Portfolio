...
# Row 6: Table
def writing_to_ppv_excel_file(case_name, 
                              ppvm_df, 
                              ppvs_df, 
                              ppv_excel_writer):
    ...
    # ppv_excel_writer = pd.ExcelWriter(f'{case_name}_ATE_Result.xlsx', engine='xlsxwriter')


    # Feed the data to the Excel writer
    ppvm_df.to_excel(ppv_excel_writer, sheet_name=f"{case_name} - PPV", startrow=3, header=False, index=False)
    rows_jumped =ppvm_df['VID'].count()
    next_title_row = rows_jumped + 5
    next_start_row = rows_jumped + 6
    ppvs_df.to_excel(ppv_excel_writer, sheet_name=f"{case_name} - PPV", startrow=next_start_row+1, header=False, index=False)

    # Create an Excel workbook
    ate_workbook  = ppv_excel_writer.book
    # Create an Excel worksheet with case_name title
    ate_worksheet = ppv_excel_writer.sheets[f"{case_name} - PPV"]
    ...
    ate_worksheet.set_column('B:D', 20)
    ate_worksheet.set_column('E:E', 300)
    ...
    ate_worksheet.write(1, 0, f'PPVm Results', cell_format_title)
    ate_worksheet.write(next_title_row, 0, f'PPVs Results', cell_format_title )

    # Write the contents of the current class hot TP results
    for col_num, value in enumerate(ppvm_df.columns.values):
        ate_worksheet.write(2, col_num, value, header_format)
    
    for col_num, value in enumerate(ppvs_df.columns.values):
